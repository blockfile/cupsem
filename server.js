...
app.get('/', (req, res) => {
  res.json({
    name: 'cupsem',
    description:
      'pump.fun creator fees → buy $CUPSY + $ANSEM for holders (claims every 5 minutes)',
...
app.use((err, req, res, next) => {
  console.error('[cupsem] request error:', err);
...
async function main() {
  await db.connect();
  console.log(`[cupsem] MongoDB connected (${config.mongoDb})`);

  getSolPriceUsd().catch(() => {}); // warm the price cache for USD values

  server = app.listen(config.port, () => {
    console.log(`[cupsem] listening on http://localhost:${config.port}`);
    console.log(`[cupsem] dryRun=${config.dryRun} wallet=${walletPubkey()}`);
    if (config.walletIsEphemeral) {
      console.log('[cupsem] WARNING: using an ephemeral wallet (no WALLET_PRIVATE_KEY set) — dry run only');
    }
    scheduler.start();
  });
}
...
async function shutdown(signal) {
  console.log(`\n[cupsem] ${signal} received, shutting down`);
...
main().catch((err) => {
  console.error('[cupsem] failed to start:', err);
  process.exit(1);
});
