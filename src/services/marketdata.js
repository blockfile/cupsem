...
// the API is unreachable, so /stats never breaks.
...
async function fetchDexScreener(mint) {
  ...
  // Solana pairs where our token is the base side; pick the deepest liquidity.
  const ours = pairs
    .filter((p) => p.chainId === 'solana' && p.baseToken && p.baseToken.address === mint)
...
async function getMarketData() {
  if (!config.tokenMint) return EMPTY;
  const now = Date.now();
  if (now - cache.at < TTL_MS && cache.at !== 0) return cache.value;
  try {
    cache = { value: await fetchDexScreener(config.tokenMint), at: now };
  } catch (_err) {
